...
map_block(u_int blockno)
{
	...
	//all pages should be mark as library to share between processes.
}
...
unmap_block(u_int blockno)
{
	...
	if(!block_is_mapped(blockno)){
		//writef("unmap_block:target block is not mapped\n");
		return;
	}
	...
	if(r<0){
		writef("unmap_block:syscall unmap error\n");
	}
...
free_block(u_int blockno)
{
	...
	if(blockno==0){
		writef("free_block:blockno = 0\n");
		return ;
	}
	if(super==NULL || blockno>=super->s_nblocks){
		writef("error at free_block\n");
		return;	
	}
...
dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	for (i = 0; i < nblock; i++) {
		...
		if(r<0){
			writef("dir_lookup:error at file_get_block()\n");
			return -E_INVAL;
		}
		...
		for(j=0;j<FILE2BLK;j++){
			if(strcmp((char*)f[j].f_name,name)==0){
				*file=&(f[j]);
				f[j].f_dir=dir;//why? why we modify it here?
				return 0;
			}
		}
	}
