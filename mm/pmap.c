...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
	...
	//get the pointer of the corresponding page directory entry
	pgtable=(Pte*)(KADDR(PTE_ADDR(*pgdir_entryp)));
	// use the PTE_ADDR to delete permission bit and get the physical address
	//ude the KADDR to switch to the kernal virtual address,which is the pointer of the corresponding page table 
	
    /* Step 2: If the corresponding page table is not exist and parameter `create`
     * is set, create one. And set the correct permission bits for this new page
     * table. */
	if((((*pgdir_entryp)&PTE_V)==0x0)&& create){  
	//if the content  the page dictory entry is not valid and allow create
		pgtable=(Pte*)alloc(BY2PG,BY2PG,1);
		//here we don't need to worry about no memory 
		// because if so we should have already called panic
		*pgdir_entryp=PADDR(pgtable)|PTE_V;
	}
	else if(((*pgdir_entryp)&PTE_V)==0x0){ 
		return NULL;	
	}	

    /* Step 3: Get the page table entry for `va`, and return it. */
	pgtable_entry=(Pte*)(pgtable+PTX(va));
	//calculate the pointer of corresponding page table entry
...
//question can we assure that the va is aligned by BY2PG?
// according to the usage of the function in the shown code,yeah,but...
}
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
	...
	if((((*pgdir_entryp)&PTE_V)==0x0)&&create){
		// if page directory is invalid and we allow create
		status=page_alloc(&ppage);//alloc a new page as the missing page table
		if(status!=0){
			// if we can't create
			*ppte=NULL;
			return -E_NO_MEM;
		}
		*pgdir_entryp=page2pa(ppage)|PTE_V;//fill in the page directory
		pgtable=(Pte*)(page2kva(ppage));// get the new pagetable pointer
		ppage->pp_ref+=1;// increase the reference
	
	}
	else if(((*pgdir_entryp)&PTE_V)==0x0){
		// if invalid and we don't allow create we just go die
		*ppte=NULL;
		return -E_NO_MEM;
	}
...
page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
	...
	//unlike the boot stage, it's possible to get a null pointer new page.
    ...
    /* Step 2: Update TLB. */
	tlb_invalidate(pgdir,va);// actually it should be called make TLB invalid
    /* hint: use tlb_invalidate function */


    /* Step 3: Do check, re-get page table entry to validate the insertion. */
	 status=pgdir_walk(pgdir, va, 1, &pgtable_entry);
    /* Step 3.1 Check if the page can be insert, if can’t return -E_NO_MEM */
	if(status!=0) { return -E_NO_MEM; }
