...
read(int fdnum, void *buf, u_int n)
{
	...
	// Step 2: Check open mode.if ((fd->fd_omode & O_ACCMODE) == O_RDONLY) 
	if ((fd->fd_omode & O_ACCMODE) == O_WRONLY) {
		writef("[%08x] read %d -- bad mode\n", env->env_id, fdnum);
		return -E_INVAL;
	}

	// Step 3: Read starting from seek position.if (debug) writef("write %d %p %d via dev %s\n",
 //writef("read fdnum:%d buf:%x size:%d via dev %s\n",
...
write(int fdnum, const void *buf, u_int n)
{
	...
	//writef("============%s\n",dev->dev_name);
	// writef("write fdnum:%d buf:%x size:%d via%s  \n",  fdnum, buf, n,dev->dev_name);
