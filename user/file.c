...
open(const char *path, int mode)
{
	...
	r=fd_alloc(&fd);
	if(r<0){
		//writef("user/file.c:open:alloc fd error\n");
		return r;	
	}
	// Step 2: Get the file descriptor of the file to open.
	r=fsipc_open(path,mode,fd);
	if(r<0){
		//writef("user/file.c:open:fail in fsipc_open when opening %s\n",path);
		return r;
	}
	...
	for(i=0;i<size;i+=BY2PG){
		...
		if(r<0){
		//	writef("user/file.c:open:fail in fsipc_map when opening %s\n",path);
			return r;
		}
	}
